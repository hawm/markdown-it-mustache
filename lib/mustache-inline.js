...
function tokenizeMustacheInline(state, silent) {
  ...
  if (!silent) {
    // inline block
    token = state.push("mustache_inline_open", "span", 1);
    token.meta = { tag };
    ...
    state.push("mustache_inline_close", "span", -1);
  }
...
function renderMustacheInline(tokens, idx, options, env, renderer) {
  ...
  return `<${tag} class="mustache mustache-inline" data-mustache-name="${mustache.name}" \
  data-mustache-mark="${mustache.mark}" data-mustache-raw="${mustache.raw}"></${tag}>`;
}
...
function renderMustacheInlineBlock(tokens, idx, options, env, renderer) {
  ...
  const mustache = token.meta.tag;
  return `<${tag} class="mustache mustache-inline mustache-inline-block" \
  data-mustache-name="${mustache.name}" data-mustache-mark=${mustache.mark} \
  data-mustache-raw="${mustache.raw}">`;
}
...
export function mustacheInline(
  ...
  md.renderer.rules.mustache_inline_open = renderInlineBlock;
  md.renderer.rules.mustache_inline_close = renderInlineBlock;
}
